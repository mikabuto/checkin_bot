...
async def command_start(message : types.Message):
	...
	if(db.if_user_id_exist(message.from_user.id)):
		await message.answer("Менюшечка", reply_markup=kb.base_kb_client)
	else:
...
async def event_info_callback(callback : types.CallbackQuery):
	...
	await callback.message.edit_text(kb.create_event_description_message(kb.search_event(id_, event_id)), reply_markup=kb.create_inkb_register(id_, event_id))
...
async def event_info_callback(callback : types.CallbackQuery):
	event_id = int(callback.data.split('_')[4])
	id_ = callback.message.chat.id
	await callback.message.edit_text(kb.create_event_description_message(kb.search_event(id_, event_id)), reply_markup=kb.create_inkb_checkin(id_, event_id))
	await callback.answer()
...
async def unregister_callback(callback : types.CallbackQuery):
	...
	if (db.unregister_user(user_id, event_id)):
		await callback.message.edit_text(kb.create_event_description_message(event), reply_markup=kb.create_inkb_register(user_id, event_id))
	else:
		await callback.message.answer('Не получилось отменить регистрацию, попробуйте ещё раз', reply_markup=kb.create_inkb_register(user_id, event_id))
		await callback.message.edit_text(kb.create_event_description_message(event), reply_markup=kb.create_inkb_register(user_id, event_id))
...
async def unregister_callback(callback : types.CallbackQuery):
	...
	if (db.register_user(user_id, event_id)): 
		...
		await callback.message.answer(kb.create_event_description_message(event), reply_markup=kb.create_inkb_register(user_id, event_id))
...
@dp.callback_query_handler(Text(startswith='check_unregister_user_'))
async def check_unregister_callback(callback : types.CallbackQuery):
	event_id = int(callback.data.split('_')[3])
	user_id = callback.message.chat.id
	event = kb.search_event(user_id, event_id)
	if (db.unregister_user(user_id, event_id)):
		await callback.message.edit_text(kb.create_event_description_message(event), reply_markup=kb.create_inkb_checkin(user_id, event_id))
	else:
		await callback.message.answer('Не получилось отменить регистрацию, попробуйте ещё раз', reply_markup=kb.create_inkb_checkin(user_id, event_id))


@dp.callback_query_handler(Text(startswith='check_register_user_'))
async def unregister_callback(callback : types.CallbackQuery):
	event_id = int(callback.data.split('_')[3])
	user_id = callback.message.chat.id
	event = kb.search_event(user_id, event_id)
	if (db.register_user(user_id, event_id)):
		...
		await callback.message.edit_text('Не получилось зарегистрироваться, попробуйте ещё раз')
		await callback.message.answer(kb.create_event_description_message(event), reply_markup=kb.create_inkb_checkin(user_id, event_id))
